...
UPI Key、Client ID、CDK 池等运行配置需要在侧边栏重新填写；填写后会保存在当前浏览器本地存储。
...
4. `兑换轮数`
   - 首轮结束后，失败账号会按轮继续；同一轮每个账号只尝试一张 CDK，失败后释放 CDK 并切换下一个账号。
...
4. 后端手动取消后，账号自动回到待兑换，CDK 回到可用池。
5. 失败账号会释放旧 CDK；同一轮不会继续给同一个账号换 CDK，而是切换下一个账号。
6. 每次账号尝试都会刷新账号和 CDK 的对应关系。
