...
本文档说明当前 `CDK Redeem Only V0.2.10` 的配置和常用操作。项目仓库与更新检查已统一为：
...
3. `CDK 池`
   ...
   - 可以像邮箱池一样粘贴导入。
   - 导入后只刷新状态，不会自动兑换。
   - 如需兑换，在 Free 组手动点击 `一键兑换 UPI`、`一键兑换 IDEAL` 或 `一键兑换全部`。
...
手动点击一键兑换后，失败账号会按配置轮数继续匹配 CDK；普通刷新和导入不会自动重新提交。
...
可用 CDK 数量不足。导入新 CDK 后账号仍保留在 Free，需要手动点击 `一键兑换 UPI`、`一键兑换 IDEAL` 或 `一键兑换全部` 继续。
