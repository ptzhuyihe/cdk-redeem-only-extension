      ...
      if (status === 'free') {
        ...
        if (redeemStatus === 'canceled') {
          return {
            className: 'pending',
            label: '已取消',
            detail: row.redeemReason || row.reason || '后端已取消兑换；如需继续请手动重新兑换。',
          };
        }
        if (redeemStatus === 'blocked' || isPreSubmitUpiCredentialMembershipBlockedRow(row)) {
        ...
        if (redeemStatus === 'failed') {
          ...
          return {
            ...
            detail: `${row.redeemReason || row.reason || '历史 CDK 兑换失败'}；${progressText}；${trialStatus === 'eligible' ? '账号有试用资格。' : '账号保留在 Free。'}`,
          };
        }
